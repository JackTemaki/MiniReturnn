...
class SprintNnPythonLayer:
    ...
    def setInputDimension(self, stream, size):
        ...
        """
        print(
            "RETURNN SprintControl[pid %i] SprintNnPythonLayer.setInputDimension: stream=%r, size=%r"
    ...
    def forward(self, input):
        ...
        if Verbose:
            print(
                "RETURNN SprintControl[pid %i] SprintNnPythonLayer.forward: %s"
    ...
    def backpropagate(self, errorSignalIn):
        ...
        if Verbose:
            print(
                "RETURNN SprintControl[pid %i] SprintNnPythonLayer.backpropagate: %r"
                % (os.getpid(), errorSignalIn.shape)
            )
...
class PythonControl:
    ...
    def init_processing(self, input_dim=None, output_dim=None, **kwargs):
        ...
        """
        print(
            "RETURNN SprintControl[pid %i] init_processing input_dim=%r, output_dim=%r"
    ...
    def process_segment(self, name, orthography, features=None, alignment=None, soft_alignment=None, **kwargs):
        ...
        if Verbose:
            print(
                "RETURNN SprintControl[pid %i] process_segment name=%r orth=%r"
        ...
        loss, error_signal = self._get_loss_and_error_signal_via_sprint_callback(
    ...
    def _get_loss_and_error_signal_via_sprint_callback(self, seg_name, orthography, posteriors):
        ...
        loss, error_signal = self.sprint_callback(
            ...
            orthography=orthography,  # Sprint wants shape (dim,time).
    ...
    def _handle_cmd_get_loss_and_error_signal(self, seg_name, seg_len, posteriors):
        ...
        if Verbose:
            print(
                "RETURNN SprintControl[pid %i] PythonControl handle_cmd_get_loss_and_error_signal: name=%r, len=%r"
    ...
    def run_control_loop(self, callback, **kwargs):
        ...
        print(
            "RETURNN SprintControl[pid %i] PythonControl run_control_loop control: %r"
            % (os.getpid(), callback("version"))
        )
    ...
    def check_control_loop_running(self):
        ...
        if self.control_loop_started:
            print(
                "RETURNN SprintControl[pid %i] PythonControl check_control_loop_running: already running"
                % (os.getpid(),)
            )
            return
    ...
    def _wait_for_control_loop_error_signal(self):
        while True:
            with self.cond:
                ...
                if Verbose:
                    print(
                        "RETURNN SprintControl[pid %i] getSegmentList: wait for control loop to handle error signal"
                        % (os.getpid(),)
                    )
                self.cond.wait(timeout=1)
        ...
        while True:  # outer loop
            ...
            with self.cond:
                ...
                if not (self.control_thread__have_new_error_signal or self.control_thread__have_new_seg):
                    print(
                        "RETURNN SprintControl[pid %i] getSegmentList, no error signal, skip segment: %s"
                        % (os.getpid(), seg_name)
                    )
                    if Verbose:
                    ...
                    if not self.notified_for_segment:
                        print(
                            (
                                "RETURNN SprintControl[pid %i] getSegmentList: "
                                "Do you use PythonControl in the Sprint trainer? Got no segment notification."
                            )
                            % (os.getpid(),)
                        )
                    if not self.asked_for_posteriors:
                        print(
                            (
                                "RETURNN SprintControl[pid %i] getSegmentList: "
                                "Do you use PythonLayer in Sprint? Did not get asked for posteriors."
                            )
                            % (os.getpid(),)
                        )
